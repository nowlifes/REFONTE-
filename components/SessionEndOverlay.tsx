...
const SessionEndOverlay: React.FC<SessionEndOverlayProps> = ({ onViewReport, nickname }) => {
  ...
  return (
    <div className="fixed inset-0 z-[1000] bg-navy-950/95 backdrop-blur-xl flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-500">
      ...
      <p className="text-slate-400 font-mono text-[10px] uppercase tracking-[0.3em] mb-8 max-w-[250px] leading-relaxed">
      ...
      {top3.length > 0 && (
        <div className="w-full max-w-xs bg-white/5 border border-white/10 rounded-3xl p-6 mb-8 relative overflow-hidden">
           ...
           <h3 className="text-[10px] font-impact text-gold-500 uppercase tracking-[0.4em] mb-6">The Legends</h3>
           ...
           <div className="flex items-end justify-center gap-3 h-24">
              ...
              {top3[1] && (
                <div className="flex-1 flex flex-col items-center">
                  ...
                  <div className="w-full bg-slate-400/20 border border-white/10 h-10 rounded-t-lg flex items-center justify-center">
                     <Medal className="w-3 h-3 text-slate-400" />
                  </div>
                  <span className="text-[7px] font-mono text-slate-500 uppercase mt-1 truncate w-full">{top3[1].pseudo}</span>
                </div>
              )}
              {/* 1st */}
              {top3[0] && (
                <div className="flex-1 flex flex-col items-center">
                  ...
                  <div className="w-full bg-gold-500/20 border border-gold-500/30 h-14 rounded-t-lg flex items-center justify-center">
                     <span className="text-xs font-black text-gold-500">{top3[0].score}</span>
                  </div>
                  <span className="text-[8px] font-mono text-gold-500 uppercase mt-1 truncate w-full font-bold">{top3[0].pseudo}</span>
                </div>
              )}
              {/* 3rd */}
              {top3[2] && (
                <div className="flex-1 flex flex-col items-center">
                  ...
                  <span className="text-[7px] font-mono text-slate-500 uppercase mt-1 truncate w-full">{top3[2].pseudo}</span>
                </div>
              )}
           </div>
        </div>
      ...
      <div className="w-full max-w-xs space-y-4">
        <button 
          ...
          className="w-full bg-gold-500 hover:bg-gold-400 text-navy-950 font-impact text-xl uppercase py-5 rounded-2xl tracking-[0.2em] shadow-[0_0_30px_rgba(234,179,8,0.2)] flex items-center justify-center gap-3 active:scale-95 transition-all"
        >
